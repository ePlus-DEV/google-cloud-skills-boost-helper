...
Run `npx react-doctor@0.2.16 --verbose --scope changed` and check the score did not regress.

If the score dropped, fix the regressions before committing.

## For general cleanup or code improvement:

Run `npx react-doctor@0.2.16 --verbose` (the default `--scope full`) to scan the full codebase. Fix issues by severity — errors first, then warnings.
...
5. Re-run `npx react-doctor@0.2.16 --verbose --scope changed`, then the project type checks and tests.
...
```bash
npx react-doctor@0.2.16 --verbose --scope changed
```
